...
if( !function_exists( 'wpbs_get_template' ) ) :
    /**
    ...
    function wpbs_get_template( $template_name, $args = array(), $template_path = '' ) {
        if( empty( $template_path ) ) :
            $template_path = WPBS_PATH . 'templates/';
        endif;        
        ...
        if ( ! file_exists( $template ) ) :
            return new WP_Error( 
                ...
                sprintf( 
                    ...
                    esc_html__( '%s does not exist.', 'blocksy-slider' ), 
                    '<code>' . esc_html( $template ). '</code>' 
                ) 
            );
        endif;

        do_action( 'wpbs_before_template_part', $template, $args, $template_path );
