...
<div class="gtbs_slides_wrap">
    <ul class="gtbs_slides">
        <?php
        if ( isset( $slidesData ) && is_array( $slidesData ) && ! empty( $slidesData ) ) :
            ...
            foreach ( $slidesData as $slide ) :
                ...
                <li data-slide-id="<?php echo esc_attr( $slide_id ); ?>">
                    ...
                    ?>
                        <div class="gtbs_slide_placeholder">
                            <span class="dashicons <?php echo esc_attr( $icon ); ?>"></span>
                        </div>
                    <?php endif; ?>

                    <div class="gtbs_slide_actions">
                        <a href="<?php echo esc_url( $edit_url ); ?>" class="gtbs_slide_edit" target="_blank">
                            <span class="tooltip"><?php echo esc_html__( 'Edit', 'gutenberg-slider' ); ?></span>
                            <i class="dashicons dashicons-edit"></i>
                        </a>

                        <a href="#" class="gtbs_slide_move">
                            <span class="tooltip"><?php echo esc_html__( 'Drag & Sort', 'gutenberg-slider' ); ?></span>
                            <i class="dashicons dashicons-move"></i>
                        </a>

                        <a href="#" class="gtbs_slide_remove">
                            <span class="tooltip"><?php echo esc_html__( 'Delete', 'gutenberg-slider' ); ?></span>
                            <i class="dashicons dashicons-trash"></i>
                        </a>
                    </div>

                    <input type="hidden" name="gtbs_slide_ids[]" value="<?php echo esc_attr( $slide_id ); ?>" />
                </li>
            <?php
            endforeach;
        ...
        elseif ( isset( $imageIDs ) && ! empty( $imageIDs ) && is_array( $imageIDs ) ) :
            ...
            foreach ( $imageIDs as $imageID ) :
                $image_src = wp_get_attachment_image_src( $imageID, 'gtbs_slideshow_thumbnail' )[0] ?? '';
    ...
    wp_nonce_field( 'gtbs_slideshow_metabox_data', 'gtbs_slideshow_metabox_nonce' );
    
    if ( $is_saved ) : ?>
        <a href="<?php echo esc_url( $add_slide_url ); ?>" class="button button-large gtbs_upload_slide">
            <?php echo esc_html__( 'Add New Slide', 'gutenberg-slider' ); ?>
        </a>
    <?php else : ?>
        <p class="gtbs_add_slide_message">
            <?php echo esc_html__( 'Save your slider with a title to begin adding slides.', 'gutenberg-slider' ); ?>
        </p>
    <?php endif; ?>
</div>
