...
<td>
	<div class="wpbs_range_field">
		<div class="wpbs_range_wrap">
			<input
				type="range"
				...
				class="wpbs_input wpbs_input_range"
				oninput="this.nextElementSibling.value=this.value"
			>
			<output class="wpbs_range_value"><?php echo esc_html( $field_Val ); ?></output>
			...
			<?php if ( isset( $field['unit'] ) && is_array( $field['unit'] ) ) : ?>
				...
				<?php else : ?>
					<?php
					$options = get_option( 'wpbs_slider_option', array() );
					$selected_unit = isset( $options[ $field_Key . '_unit' ] )
						? $options[ $field_Key . '_unit' ]
						: ( $field['unit_selected'] ?? '' );
					?>
					<select name="wpbs_slider_option[<?php echo esc_attr( $field_Key . '_unit' ); ?>]">
