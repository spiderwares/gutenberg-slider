...
<td>
    <?php if ( isset( $field['options'] ) ) : ?>
        <div class="gtbs_radio_field" <?php echo isset( $field['data_hide'] ) ? 'data-hide="' . esc_attr( $field['data_hide'] ) . '"' : ''; ?>>
            <?php foreach ( $field['options'] as $optionKey => $optionImg ) : ?>
                <p class="gtbs_image_control <?php echo in_array( $optionKey, $field['disabled_options'] ?? array() ) ? 'gtbs_disabled_option' : ''; ?>">
                    <input 
                        type="radio" 
                    ...
                    <label for="<?php echo esc_attr( $field_Key . '_' . $optionKey ); ?>">
                        <img 
                            width="150" 
                            src="<?php echo esc_url( GTBS_URL . 'assets/images/options/' . $optionImg ); ?>" 
